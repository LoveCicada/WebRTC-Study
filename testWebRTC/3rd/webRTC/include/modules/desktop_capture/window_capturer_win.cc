...
#define ALIGN_16(x) ((x % 16 == 0) ? x : ((x & ~15) + 16))
#define ALIGN_4(x) ((x % 4 == 0) ? x : ((x & ~3) + 4))
...
WindowCapturerWin::CaptureResults WindowCapturerWin::CaptureFrame(
  ...
  //test ，字节对齐
  int w = original_rect.right() - original_rect.left();
  int h = original_rect.bottom() - original_rect.top();
  int w2 = ALIGN_16(w);
  int h2 = ALIGN_16(h);
  original_rect.set_width(w2);
  original_rect.set_height(h2);
  ...
  //cropped_rect.Translate(0, 0);
  std::unique_ptr<DesktopFrame> cropped_frame = //std::move(frame);
      CreateCroppedDesktopFrame(std::move(frame), cropped_rect);
		//CreateCroppedDesktopFrame(std::move(frame), original_rect);
